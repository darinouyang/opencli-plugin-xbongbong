...
cli({
    ...
    func: async (page, args) => {
        ...
        const resp = await apiCall(page, '/form/data/assign', body, commonParams);

        if (!resp.ok || !resp.data) {
            if (resp.status === 401) throw new AuthRequiredError(DOMAIN, 'Session已过期');
            throw new CommandExecutionError(`分配客户失败: ${resp.data?.msg || resp.error || 'HTTP ' + resp.status}`);
        }

        if (resp.data.code !== 1) {
            throw new CommandExecutionError(`分配客户失败: ${resp.data.msg || '未知错误'}`);
        }
